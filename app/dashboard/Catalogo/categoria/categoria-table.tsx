...
export function CategoriaTable({
    ...
    categoria: any[];
    offset: number;
    totalCatogoria: number;
}) {
    let router = useRouter();
    let categoriaPerPage = 5;
        ...
        <div>
            ...
            <CardContent>
                <Table>
                    ...
                    <TableBody>
                        {records.map((user) => (
                            <Categoria key={user.id} cate={user} />
                        ))}
                    </TableBody>
                </Table>
            </CardContent>
            ...
            <CardFooter>
                <form className="flex items-center w-full justify-between">
                    <div className="text-xs text-muted-foreground">
                        Mostrando {' '}
                        <strong>
                            {Math.max(0, Math.min(offset - categoriaPerPage, totalCatogoria) + 1)}-{offset}
                        </strong>{' '}
                        De <strong>{totalCatogoria}</strong> Usuarios
                    </div>
                    <div className="flex">
                        <Button
                            formAction={prevPage}
                            ...
                            size="sm"
                            type="submit"
                            disabled={offset === categoriaPerPage}
                        >
                        ...
                        <Button
                            ...
                            size="sm"
                            type="submit"
                            disabled={offset + categoriaPerPage > totalCatogoria}
                        >
                            Siguiente
