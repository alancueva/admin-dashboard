...
export function UnidadTable({
    ...
    const [records, setRecords] = useState(unidad);

    const handleFilter = (status: string, search: string) => {
        const lowercasedSearch = search.toLowerCase();
        const filteredData = unidad.filter(user => {
            ...
            <CardFooter>
                <form className="flex items-center w-full justify-between">
                    <div className="text-xs text-muted-foreground">
                        Mostrando {' '}
                        <strong>
                            {Math.max(0, Math.min(offset - unidadPerPage, totalUnidad) + 1)}-{offset}
                        </strong>{' '}
                        De <strong>{totalUnidad}</strong> Usuarios
                    </div>
                    <div className="flex">
                        <Button
                            formAction={prevPage}
                        ...
                        <Button
                            ...
                            size="sm"
                            type="submit"
                            disabled={offset + unidadPerPage > totalUnidad}
                        >
                            Siguiente
